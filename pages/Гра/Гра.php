	...
	<div class="mainbox">
		<div class="content">
			<?php
	    		...
	    		if($result = $mysqli->query('SELECT * FROM Гра')) {
	    		 	printf("Дані про гру:: <br><br>");
	    		 	printf("<table><tr><th>Назва</th><th>К-сть завантажень</th><th>Графіка</th><th>Ціна</th></tr>");
	    		 	while( $row = $result->fetch_assoc() ){ 
	            	printf("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", $row['Назва'], $row['К-сть завантажень'], $row['Графіка'], $row['Ціна']); 
	        	};
	        	printf("</table>");
	        	$result->close();
	        }
